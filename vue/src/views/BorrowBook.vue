    ...
    <el-table :data="tableData" border stripe header-cell-class-name="headerBg">
      ...
      <el-table-column label="操作" align="center" width="170">
        <template slot-scope="scope">
          <el-button type="success" @click="handleBorrow(scope.row.id)">借阅 <i class="el-icon-edit"></i></el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    handleBorrow(id) {
      ...
      this.BookBorrow.borrowDate = moment(new Date()).format('YYYY-MM-DD hh:mm:ss')
      this.request.post("/book/borrow_book", this.BookBorrow).then(res => {
        if(res.code==='200')
        {
          this.load()
